...
module.export = webpack(webpackConfig).run((err, stats) => {
  ...
  chokidar.watch('.', { cwd: __dirname }).on('change', filePath => {
    console.log(`!! File ${filePath} was modified. Restarting workers...`)
    webpack(webpackConfig).run((subErr, subStats) => {
      if (subErr || subStats.compilation.errors.length > 0)
        console.error(subErr || subStats.compilation.errors)
