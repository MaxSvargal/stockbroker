...
export function* doBuySaga(symbol: string) {
  ...
  // { symbol: SYMBOL, id: ID, mts: MTS, cid: CID, price: PRICE, amount: AMOUNT, covered?: number[], profit?: number }

  // yield put(execNewOrder({ symbol, amount, price }))
  // await for cid and mts?
  yield put(createPosition({ symbol, price, amount, cid: 0, id: now(), mts: now() }))
