...
const getChunkAmount = compose(converge(divide, [ sumOfAmounts, length ]), filterOpened)
...
export default class extends Component {
  render() {
    ...
    return (
      <Div height='100%' display='flex' flexFlow='column wrap' alignItems='center' justifyContent='center'>
        <Div color='#ec407a' display='flex' flexFlow='row nowrap' alignItems='center' lineHeight='4.2rem'>
          <Small fontSize='.9rem' marginRight='.5rem'>+ balance</Small>
          <Strong fontSize='calc(0.025 * 100vw + 1rem)'> { balance.toFixed(8) } BTC</Strong>
        </Div>
        <Div fontSize='calc(14px + (26 - 14) * ((100vw - 300px) / (1600 - 300)))' color='#5c6bc0' marginBottom='3rem'>
        ...
        <Div display='flex' flexFlow='row wrap' justifyContent='center' lineHeight='1.8rem'>
          ...
          <Div>
            <div><strong>{ openedLen } / { chunksNumber }</strong></div>
